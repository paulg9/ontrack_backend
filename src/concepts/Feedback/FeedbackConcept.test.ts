...
type RecomputeOk = { summaryId: ID; newStreakCount: number; newCompletion7d: number };
...
Deno.test("FeedbackConcept: Action `recordMessage` correctly creates messages", async () => {
  ...
  const message1 = await db.collection("Feedback.messages").findOne({ _id: result1Ok.messageId });
  ...
  const message2 = await db.collection("Feedback.messages").findOne({ _id: result2Ok.messageId });
...
Deno.test("FeedbackConcept: Queries correctly handle missing summaries and existing state", async () => {
  ...
  assertEquals(metricsFailureErr.error, `Precondition failed: Summary for owner ${nonExistentOwnerId} does not exist.`);
  ...
  assertEquals(reminderCheckFailureErr.error, `Precondition failed: Summary for owner ${nonExistentOwnerId} does not exist.`);
  console.log("Requirement confirmed: Query fails as expected.");
