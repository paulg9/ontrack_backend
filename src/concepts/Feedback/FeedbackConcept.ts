  ...
  async recompute(
    ...
    try {
      ...
      if (result) {
        ...
        console.log(
          `Recomputed feedback for owner ${owner}: Summary ID ${result._id}, Streak ${result.streakCount}, Completion ${result.completion7d}`,
        );
        return {
          summaryId: result._id,
          newStreakCount: result.streakCount,
          newCompletion7d: result.completion7d,
        };
      } else {
