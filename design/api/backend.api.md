...
Purpose: Provide a catalog of exercises that a plan is allowed to reference. Administrators may request AI‑proposed details and explicitly apply or discard those proposals.

### POST /api/ExerciseLibrary/addExercise
Description: Create a fully-specified exercise.
Requirements:
- `actorIsAdmin = true`; `title` non-empty; `recommendedFreq` integer in [0, 14]
Effects:
- Creates Exercise (`deprecated := false`) and returns its id
Request Body:
```json
{
  "title": "string",
...
Description: Create a minimally specified exercise draft.
Requirements:
- `actorIsAdmin = true`; `title` non-empty
Effects:
- Creates Exercise with minimal defaults
Request Body:
```json
...
Description: Update optional exercise fields.
Requirements:
- `actorIsAdmin = true`; `exercise` exists; if provided: `title` non-empty; `recommendedFreq` integer in [0, 14]; `videoUrl` http/https or `null` to clear
Effects:
...
Description: Mark an exercise as deprecated.
Requirements:
- `actorIsAdmin = true`; `exercise` exists
Effects:
...
Description: Apply a pending proposal to its exercise.
Requirements:
- `actorIsAdmin = true`; pending `proposal` exists
Effects:
- Merges details; marks proposal applied
Request Body:
```json
...
Description: List exercises, optionally excluding deprecated.
Request Body:
```json
...
Description: List proposals for a specific exercise.
Request Body:
```json
{ "exercise": "ID" }
```
...
Description: Create a daily check-in.
Requirements:
- `actor = owner`; no existing check-in for (owner, date); strain and pain in [0..10]; `date` format `YYYY-MM-DD`
Effects:
- Creates a CheckIn
Request Body:
```json
...
Description: Update fields on an existing check-in.
Requirements:
- `checkin` exists and belongs to `actor`
Effects:
- Updates provided fields (comment cleared with null/empty string)
Request Body:
```json
{
  "actor": "ID",
...
Description: Get check-in for (owner, date).
Request Body:
```json
...
Success Response Body (Query):
```json
[ { "_id": "ID", "owner": "ID", "date": "YYYY-MM-DD", "completedItems": ["ID"], "strain_0_10": 0, "pain_0_10": 0, "comment": "string (optional)" } ]
```

### POST /api/CheckIn/_getCheckInsByOwner
Description: List all check-ins for an owner.
Request Body:
```json
{ "owner": "ID" }
```
...
Description: Check existence of a check-in for (owner, date).
Request Body:
```json
{ "owner": "ID", "date": "YYYY-MM-DD" }
```
...
Description: Create an empty plan for a user.
Requirements:
- `actor = owner`; owner has no active (non-archived) plan
Effects:
- Creates plan with empty items
Request Body:
```json
{ "actor": "ID", "owner": "ID" }
```
...
Description: Add a plan item to a plan.
Requirements:
- `plan` exists; `plan.owner = actor`; `exercise` valid and not deprecated (validated externally)
Effects:
- Appends the plan item
Request Body:
```json
{
  "actor": "ID",
...
Description: Remove a plan item by exercise id.
Requirements:
- `plan` exists; `plan.owner = actor`; item for `exercise` exists
Effects:
- Removes the item
Request Body:
```json
...
Description: Get the active (non-archived) plan for an owner.
Request Body:
```json
{ "owner": "ID" }
```
...
[
  {
    ...
    "owner": "ID",
    "archived": false,
    "items": [ { "exercise": "ID", "perWeek": 0, "sets": 0, "reps": 0, "notes": "string" } ]
  }
]
```
...
Description: Upsert summary metrics for a user.
Requirements:
- `owner` exists
Effects:
...
Request Body:
```json
{ "owner": "ID", "today": "ISO-8601 string", "newStreakCount": 0, "newCompletion7d": 0 }
```
Success Response Body:
```json
...
Description: Record a message for a user.
Requirements:
- `owner` exists
Effects:
...
Description: System action to send a reminder and record it.
Requirements:
- Summary for `owner` exists
Effects:
- Records reminder Message; updates `lastReminderDate`
Request Body:
```json
{ "owner": "ID" }
```
...
Description: Get streak and 7‑day completion ratio.
Request Body:
```json
{ "owner": "ID" }
```
...
Description: Check if a reminder was sent on a given date.
Request Body:
```json
...
Description: List messages for an owner (newest first).
Request Body:
```json
{ "owner": "ID" }
```
...
```json
[
  { "_id": "ID", "owner": "ID", "timestamp": "ISO-8601 string", "kind": "reminder|motivation|summary", "text": "string" }
]
```
...
- `scaleMin < scaleMax`
Effects:
- Creates survey and returns its id
Request Body:
```json
...
- Survey exists
Effects:
- Creates question and returns its id
Request Body:
```json
...
Requirements:
- Question exists; response exists; value within scale
Effects:
...
Description: List all answers by a respondent.
Request Body:
```json
{ "respondent": "ID" }
```
...
Effects:
- Creates User; sets default fields
Request Body:
```json
...
Description: Sign in and receive a session token.
Requirements:
- A user with `username` exists and `password` matches
Effects:
- Creates a Session with token; returns token
Request Body:
```json
...
Description: Sign out by invalidating a session token.
Requirements:
- Session with `token` exists
Effects:
- Deletes the Session
Request Body:
```json
...
Description: Set a user’s reminder time.
Requirements:
- User exists
Effects:
- Sets `reminderTime := time`
Request Body:
```json
{ "user": "ID", "time": "HH:MM" }
```
...
Description: Resolve a user id from an active session token.
Request Body:
```json
{ "token": "string" }
```
...
Description: Return whether a user is an administrator.
Request Body:
```json
{ "user": "ID" }
```
...
Description: Return whether a token corresponds to an active (non-expired) session.
Request Body:
```json
{ "token": "string" }
```
...
Description: Create a share link for a user with TTL.
Requirements:
- Owner exists
Effects:
- Creates ShareLink; returns token
Request Body:
```json
...
Description: Revoke a share link by token for the owner.
Requirements:
- ShareLink with `token` exists and belongs to `owner`
Effects:
- Removes ShareLink and detaches from owner
Request Body:
```json
